"""
Real implementation of DHT1 (Temperature and Humidity Sensor - Bedroom) using RPi.GPIO
"""
import time
import threading
...
def run_dht1_loop(dht1_instance, delay, callback, stop_event):
    """
    Run continuous loop reading DHT sensor.
    
    Args:
        dht1_instance: DHT1 instance
    ...
    try:
        while not stop_event.is_set():
            check = dht1_instance.readDHT11()
            code = parseCheckCode(check)
            humidity, temperature = dht1_instance.humidity, dht1_instance.temperature
            callback(humidity, temperature, code)
            time.sleep(delay)
    except KeyboardInterrupt:
        print('\n[DHT1] Measurement stopped by user')
        dht1_instance.cleanup()
    except Exception as e:
        print(f'[DHT1] Error: {str(e)}')
        dht1_instance.cleanup()
