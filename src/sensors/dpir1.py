"""
Real implementation of DPIR1 (Door Motion Sensor) using RPi.GPIO
"""
import time
import threading
...
def run_dpir1_loop(dpir1_instance, interval=0.5, callback=None, stop_event=None):
    """
    Run continuous loop monitoring motion sensor.
    
    Args:
        dpir1_instance: DPIR1 instance
    ...
    try:
        while not stop_event.is_set():
            current_state = dpir1_instance.get_state()
        ...
        print('\n[DPIR1] Stopped by user')
        dpir1_instance.cleanup()
    except Exception as e:
        print(f'[DPIR1] Error: {str(e)}')
        dpir1_instance.cleanup()
