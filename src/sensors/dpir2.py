"""
Real implementation of DPIR2 (Door Motion Sensor) using RPi.GPIO
"""
import time
import threading
...
def run_dpir2_loop(dpir2_instance, interval=0.5, callback=None, stop_event=None):
    """
    Run continuous loop monitoring motion sensor.
    
    Args:
        dpir2_instance: DPIR2 instance
    ...
    try:
        while not stop_event.is_set():
            current_state = dpir2_instance.get_state()
        ...
        print('\n[DPIR2] Stopped by user')
        dpir2_instance.cleanup()
    except Exception as e:
        print(f'[DPIR2] Error: {str(e)}')
        dpir2_instance.cleanup()
